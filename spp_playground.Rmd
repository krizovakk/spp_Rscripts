...
-- prevod DT z hodinovych dat na 15minutu
...
```
# MARIUS PEDERSEN diagram

mail Market 2.10.2025
...
mysheets <- read_excel_allsheets("C:/Users/krizova/Documents/playground/marius_pedersen/mp.xlsx") # specifikuj excel file

library(dplyr)

df_2col <- mysheets %>%
  purrr::imap(~ mutate(.x, source = .y)) %>%
  purrr::keep(~ ncol(.x) == 3) %>%
  bind_rows()

df_2col$`Datum od` <- factor(df_2col$`Datum od`)

ctvrt <- df_2col %>% 
  group_by(`Datum od`) %>% 
  summarise(kWh = sum(as.numeric(Výroba)))

library(writexl)
write_xlsx(x = ctvrt,                       
           path = "C:/Users/krizova/Documents/playground/marius_pedersen/MP_spotreby15min.xlsx")
```
