...
update.packages(checkBuilt = TRUE, ask = FALSE) # update all
```
...
```{r}
excel_value <- read_excel(excel_file, sheet = "Output", range = "P22", col_names = FALSE)[[1]]
```
...
scientific format to be avoided using this command at the beginning of the code
...
### rename column usinf abother variable
...
```{r}
futu <- df %>% 
  mutate(trad_date = as.Date(datum)) %>% 
  add_row(trad_date = max(.$trad_date) + 1) 
```
...
g1 +
  ...
  scale_y_continuous(
    limits = c(30, 40),
    breaks = seq(30, 40, by = 0.5),
    labels = function(x) paste0(x, " €"))+
  scale_x_date(
    limits = c(Sys.Date() %m-% months(3), Sys.Date()), # for last three months (lubridate)
    breaks = "1 week", # 2 days works also fine
    date_labels = "%d-%b",
    minor_breaks = NULL,
    expand = c(0, 1)) # no prolongation of x axis
```
...
Caution!
  ...
  -> theme() as the last element ALWAYS
...
## vypocet 3sigma pro odstraneni outlieru

```{r}

mean_x <- mean(df$values)
sd_x <- sd(df$values)

# 3-sigma limits
print(lower_limit <- mean_x - 3 * sd_x)
print(upper_limit <- mean_x + 3 * sd_x)
```
