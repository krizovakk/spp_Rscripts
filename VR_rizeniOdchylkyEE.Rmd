...
tri sigma lze pouzit jen pro normalni rozlozeni dat
zde mame silne extremy -> neprukazne
zkousela jsem vice moznosti, nejlepe vychazi kvantily

```{r}
sigm <- dat %>% 
  select(id, datum, hodina, ctvrthodina, naklad_energas, naklad_energodata, naklad_powerex)

g <- sigm %>% select(naklad_energas) %>% arrange(naklad_energas) %>% na.omit()

# Quantile
lower_q <- quantile(g$naklad_energas, 0.01)   # bottom 1%
upper_q <- quantile(g$naklad_energas, 0.99)   # top 1%

# outliers_q <- g$naklad_powerex[g$naklad_powerex < lower_q | g$naklad_powerex > upper_q]
